...
# gif TERMINAL
set term gif animate delay 5 loop 1\
    font "Arial,18pt" \
    size 1100,800
set output "epidemic_mask.gif"
...
stats 'Nwalkers.dat' name 'b'
    set label 1 sprintf("N_{w,initial} = %i", b_max_y) at graph 0., screen 0.98
    set label 2 sprintf("N_{w,final} = %i", b_min_y) at graph 0., screen 0.94

set label 3 "N_{w,masks}/N_{w,initial} ≈ 80%" at graph 0.8, screen 0.98
set label 4 "Mask protection = 50%" at graph 0.8, screen 0.94
#set label 3 "N_{w,masks}/N_{w,initial} = 20%" at graph 0.8, screen 0.98
